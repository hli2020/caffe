...
void read_image(std::ifstream* file, int* coarse_label, int* label, char* buffer) {
...
void convert_dataset(const string& input_folder, const string& output_folder,
  ...
  for (int fileid = 0; fileid < kCIFARTrainBatches; ++fileid) {
    ...
    snprintf(str_buffer, kCIFARImageNBytes, "/train_%d.bin", fileid + 1);
...
int main(int argc, char** argv) {
  if (argc != 4) {
    printf("This script converts the CIFAR-100 dataset to the leveldb/lmdb format used\n"
           "by caffe to perform classification.\n"
           "Usage:\n"
