  ...
  for ( int cycle = 0; cycle < read_cycles ; ++cycle ) {
    ...
    for ( int item_id = 0; item_id < batch_size; ++item_id ) {
      ...
      if (datum.encoded()) {
        ...
        if ( (read_cycles != (cycle - 1) ) && (0 < this->layer_param_.data_param().shuffle_buffer_size())) {
          
          Dtype * shuffle_buffer_data = this->shuffle_buffer_data_.mutable_cpu_data() + 
            this->shuffle_buffer_data_.offset(cycle * this->prefetch_data_.num());
